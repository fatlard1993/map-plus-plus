#!/usr/bin/env python3
"""Generate Map++'s mod menu icon: the vanilla filled map.
...
OUT = os.path.join(HERE, "src/main/resources/assets/map-plus-plus/icon.png")
...
def build_icon():
    """The mod's own textures are empty slot backgrounds, which are invisible
    at icon size. A filled map is the subject, so that is the icon."""
    return scale(vanilla("item/filled_map.png"), 8)
