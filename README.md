...
It is a treasure enchantment: found on books in loot and sold by librarians. A map cannot go in an enchanting table at all - vanilla gives maps no enchantability, so the table will not take one - which makes the book and an anvil the only way onto one.
